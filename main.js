...
var gDirection = new THREE.Vector3(0, -1, 0);
...
//音源绑定到一个网格模型上
timePlane.add(PosAudio);
// 创建一个音频加载器
var audioLoader = new THREE.AudioLoader();
// 加载音频文件，返回一个音频缓冲区对象作为回调函数参数
audioLoader.load('./main.mp3', function(AudioBuffer) {
    // console.log(buffer);
    // 音频缓冲区对象关联到音频对象audio
    PosAudio.setBuffer(AudioBuffer);
    PosAudio.setVolume(0.05); //音量
...
function init() {
    ...
    const onKeyDown = function(event) {
        ...
        switch (event.code) {
            ...
            case 'Space':
                if (canJump === true) velocity.y -= 350 * gDirection.normalize().y;
                canJump = false;
                break;
    ...
    let floorGeometry = new THREE.SphereGeometry(earthRadius, 32, 32);
...
function collictionTest() {
    ...
    return beforeObject || asideObject;
}

function gravityCompute(delta) { //earth center is (0,-1000,0)
    const earthCenter = new THREE.Vector3(0, -earthRadius, 0);
    gDirection = earthCenter.sub(controls.getObject().position);
    gDirection.normalize();
    controls.getObject().up = new THREE.Vector3(0, 0, 0).sub(gDirection);
    velocity.y += gDirection.y * 9.8 * 100.0 * delta;
}
